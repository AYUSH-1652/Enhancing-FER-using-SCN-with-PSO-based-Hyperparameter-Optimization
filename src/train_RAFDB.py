...
class RafFolderDataset(data.Dataset):
    def __init__(self, root_dir, phase="train", transform=None,
        ...
        if len(self.samples) == 0:
            raise RuntimeError(f"No images found for phase={phase}")
...
def create_dataloaders(args):
    ...
    full_train_dataset = RafFolderDataset(
...
def train_model(base_args, params, epochs, save_path=None, final_mode=False):
    ...
    (train_loader, val_loader, test_loader,
     train_dataset, val_dataset, test_dataset) = create_dataloaders(base_args)
    ...
    for epoch in range(1, epochs + 1):
        ...
        if epochs_no_improve >= base_args.patience:
            print(f"\nEarly stopping after {base_args.patience} "
    ...
    if save_path is not None and os.path.exists(save_path):
        print(f"\nLoading best model from {save_path} for official test ...")
        ...
        test_loss, test_acc = evaluate(
            ...
            use_tta=base_args.use_tta_test
        )
        print(f"Final official TEST accuracy: {test_acc:.4f}  "
              f"loss: {test_loss:.4f}")
...
def final_training(base_args, best_params):
    ...
    final_save_path = os.path.join(
        base_args.save_dir, "best_RAFDB.pth"
    )
    ...
    with open(os.path.join(base_args.save_dir, "best_params_pso_scratch.json"),
              "w", encoding="utf-8") as f:
        json.dump(best_params, f, indent=2)

    print("\nFinal model saved as best_RAFDB.pth")
...
def run_training():
    ...
    if args.run_pso:
        ...
        np.save(
            os.path.join(args.save_dir, "best_params_pso_scratch.npy"),
            best_params
        )
